...
const RoadLimitationType: FC<{
    ...
    return (
        <Box sx={{
            ...
            px: 5.5, height: 50, borderRadius: 4, border: '1px solid rgba(132, 89, 255, 0.2)',
        }}
        >
            <Box
                sx={{
                    width: 140, height: 40, top: 5, transition: 'all ease-out 0.4s',
                    position: 'absolute', backgroundColor: value === 'TWO_WAY' ? '#8459FF' : '#FF3030',
                    left: value === 'TWO_WAY' ? 5 : 135 }}
            />
            <Typography
                onClick={() => {onChange('TWO_WAY')}}
                sx={{ color: value === 'TWO_WAY' ? '#FFF' : '#909090' }}
            ...
            <Typography
                ...
                sx={{ color: value === 'BLOCKED' ? '#FFF' : '#909090' }}
                fontWeight={500} fontSize={16} lineHeight="16px"
            >
                Blocked
            </Typography>
    ...
    return (
        <Box sx={{ mt: -1, width: 1, mb: 3.5 }} >
            <Box sx={{ display: 'flex', mb: 1.25 }} >
                <Typography fontWeight={400} fontSize={16} lineHeight="16px">Toll amount</Typography>
            </Box>
            <Box sx={{ display: 'flex', width: 1, alignItems: 'center', mb: 3.75 }} >
                ...
                <IconButton onClick={onEdit} >
                    <EditIcon sx={{ color: '#8459FF', fontSize: '2rem' }} />
                </IconButton>
            </Box>
            <Box sx={{ display: 'flex', mb: 1.25 }} >
                <Typography fontWeight={400} fontSize={16} lineHeight="16px">Limitation</Typography>
            </Box>
