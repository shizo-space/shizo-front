...
const EvmWalletProvider: FC<EvmWalletProviderProps> = ({ children, chains = evmChains }) => {
  ...
  const isWalletInstalled = useMemo<boolean>(
    () => !!(typeof evmWallet !== 'undefined' && evmWallet.isMetaMask),
    [evmWallet],
  )
  ...
  const {
    ...
    (tokenAddress, symbol, image = undefined, decimals = 18) =>
      addTokenToWallet(tokenAddress, symbol, image, decimals),
    {
      manual: true,
    },
  )
  ...
  const activeWalletAddress = useMemo<string | null>(
    () => (isWalletReady ? userWalletAddress : null),
    [userWalletAddress, isWalletReady],
  )
