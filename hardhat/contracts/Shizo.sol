...
contract Shizo is ERC721 {
	...
	uint256 public constant MAX_DELTA_TIME = 24 * 3600; //max deltaTime for a transit
	...
	mapping(uint8 => uint24) teleportCooldown;

	mapping(uint8 => uint256) shenRequiredForUpgrade;
	...
	struct TransitStorage {
		mapping(uint256 => TransitStep) steps;
		uint8 t;
		uint256 stepsCount;
	...
	struct RoadBlockStorage {
		uint256 propsCount;
		uint256 lastPropIndex;
		mapping(uint256 => RoadBlockProps) props; // TODO ouside of the struct
	}
	...
	constructor(address _shenAddress) ERC721("Shizo", "Shizo") {
		...
		teleportCooldown[3] = 30 * 60;
		teleportCooldown[4] = 10 * 60;
		teleportCooldown[5] = 5 * 60;

		shenRequiredForUpgrade[1] = 25 * (10**decimals);
		shenRequiredForUpgrade[2] = 50 * (10**decimals);
		shenRequiredForUpgrade[3] = 100 * (10**decimals);
		shenRequiredForUpgrade[4] = 250 * (10**decimals);
		shenRequiredForUpgrade[5] = 500 * (10**decimals);
	...
	function mintByShen(
		...
		ERC20(shenAddress).transferFrom(
			msg.sender,
			address(this),
			shenRequiredForMint[rarity]
		);
		...
		teleportsProps[tokenId] = TeleportProps(teleportCooldown[1], 0);
	...
	function mint(
		...
		require(
			msg.value == maticRequiredForMint[rarity],
	...
	function upgrade(uint256 _tokenId) public payable {
		require(_exists(_tokenId), "upgrade: token not found");
		require(
	...
	function purchase(uint256 _tokenId) public payable {
		...
		require(
			msg.value == marketplace[_tokenId].price,
			"purchase: the price is not correct"
		);
	...
	function shizoRequiredForLvlup(uint256 tokenId)
		public
		view
		returns (uint256)
	{
	...
	function lvlup(uint256 tokenId) external {
		...
		require(
			IERC20(shenAddress).balanceOf(msg.sender) >= shenRequired,
			"Not enough SHEN"
		);
	...
	function teleport(uint256 tokenId) external {
		...
		require(
			teleportsProps[tokenId].cooldown +
				teleportsProps[tokenId].lastTeleportTime <
				block.timestamp,
	...
	function changeRoadLimitations(uint256 tokenId, bool isBlock) external {
		...
		uint256 propsCount = roadBlockStorage[tokenId].propsCount;
		uint256 lastPropIndex = roadBlockStorage[tokenId].lastPropIndex;
		if (
			propsCount > 0 &&
			roadBlockStorage[tokenId].props[lastPropIndex].isBlock == isBlock
		) {
			revert("Already applied");
		}

		uint256 oldestPropIndex = 0;
		if (propsCount > 1) {
			for (uint256 i = 1; i < propsCount; i++) {
				if (
					roadBlockStorage[tokenId].props[i].modifiedTime <
					roadBlockStorage[tokenId]
						.props[oldestPropIndex]
						.modifiedTime
				) {
					oldestPropIndex = i;
				}
		...
		if (
			currentTime -
				roadBlockStorage[tokenId].props[oldestPropIndex].modifiedTime >
			MAX_DELTA_TIME
		) {
			roadBlockStorage[tokenId]
				.props[oldestPropIndex]
		...
		emit RoadLimitationChanged(
			ownerOf(tokenId),
			tokenId,
			isBlock ? ROAD_BLOCKED : ROAD_UNBLOCKED
		);
	}

	function readLastRoadblockStatuses(uint256[] memory tokenIds)
		public
		view
		returns (bool[] memory)
	{
		...
		for (uint256 i = 0; i < tokenIds.length; i++) {
			...
			if (propsCount > 0) {
				result[i] = roadBlockStorage[tokenIds[i]]
					.props[roadBlockStorage[tokenIds[i]].lastPropIndex]
					.isBlock;
			} else {
				result[i] = false;
			}
	...
	function startTransit(
		uint8 _type,
		TransitStep[] memory steps,
		bytes memory signature
	) external {
		require(
			staticPositions[msg.sender].lat == steps[0].lat,
			"Transit: Invalid starting position"
		);
		require(
			staticPositions[msg.sender].lon == steps[0].lon,
			"Transit: Invalid starting position"
		);
		...
		for (uint256 i = 0; i < steps.length; i++) {
			...
			if (
				i > 0 &&
				roadBlockStorage[tokenId].propsCount > 0 &&
				roadBlockStorage[tokenId]
					.props[roadBlockStorage[tokenId].propsCount - 1]
					.isBlock &&
	...
	function getDistanceTraversed(address addrs)
		public
		view
		returns (uint32, uint256)
	{
		...
		uint32 deltaTime = uint32(
			block.timestamp - transits[addrs].departureTime
		);
		...
		uint256 time = transits[addrs].departureTime;
		uint256 lastStepIndex = 0;
		for (uint256 i = 0; i < transits[addrs].stepsCount; i++) {
	...
	function finishTransit() external {
		...
		uint256 stepsCount = transits[msg.sender].stepsCount;
		...
		uint256 shenRequired = shenConsumption[transits[msg.sender].t] *
			distance;
		...
		staticPositions[msg.sender].lat = transits[msg.sender]
			.steps[stepsCount - 1]
			.lat;
		staticPositions[msg.sender].lon = transits[msg.sender]
			.steps[stepsCount - 1]
			.lon;
	...
	function getTransitSteps(address addrs)
		public
		view
		returns (TransitStep[] memory)
	{
		TransitStep[] memory steps = new TransitStep[](
			transits[addrs].stepsCount
		);
		for (uint256 i = 0; i < transits[addrs].stepsCount; i++) {
